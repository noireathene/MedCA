...
public class dbManager extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {                                           //sql query to insert data in sqllite
        String query = "create table tbl_reminder(id integer primary key autoincrement,patientname text, title text,date text,time text)";
    ...
    public String addreminder(String name, String title, String date, String time) {
    ...
    public Cursor readPatientMed(String name) {
        SQLiteDatabase database = this.getWritableDatabase();
        String query = "select * from table_reminder where patientname like '"+name+"' order by id desc";                               //Sql query to  retrieve  data from the database
        Cursor cursor = database.rawQuery(query, null);
        return cursor;
    }
}
